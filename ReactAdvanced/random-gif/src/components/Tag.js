    ...
    return (
        <div className="w-1/2 bg-blue-500 rounded-lg border border-black flex flex-col items-center
...

           {
            ...
            <input 
                className="w-10/12 text-lg rounded-lg text-center py-2"
                onChange={(event) => setTag(event.target.value)} value={tag}
            />
            <button  onClick={() => fetchData(tag)} 
            className="w-10/12 bg-yellow-500 text-lg rounded-lg text-center py-2 mb-[20px]"
             >Generate</button>
        </div>
    );
}
